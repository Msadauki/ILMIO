...
app.post('/api/classes', authMiddleware, adminMiddleware, async (req,res)=>{
  ...
  try {
    const result = await pool.query(
      'INSERT INTO classes(title, description) VALUES($1,$2) RETURNING id,title,description',
      [title, description || '']
    );
...
// Admin creates exam
app.post('/api/exams', authMiddleware, adminMiddleware, async (req,res)=>{
  const { classId, title, totalMarks } = req.body;
  if (!classId || !title || !totalMarks) return res.status(400).json({ error:'Missing fields' });
  try {
    const result = await pool.query(
      'INSERT INTO exams(class_id, title, total_marks) VALUES($1,$2,$3) RETURNING id,title,total_marks,class_id',
